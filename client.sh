...
ip link set dev veth-A netns client$TARGET
...
$CLI ping 10.0.1.1 -c 1 -W 0.00001
...
$CLI tc qdisc add dev veth-A root handle 1: netem delay ${DELAY_MS}ms ${DELAY_DISTRIBUTION_MS}ms distribution normal
$CLI tc qdisc add dev veth-A parent 1: handle 2: netem loss ${LOSS_PERCENT} rate 100Mbit corrupt 1% duplicate 1%
